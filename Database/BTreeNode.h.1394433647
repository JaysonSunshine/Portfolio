...
const int headerSize = 3 * sizeof(int) + 2 * sizeof(PageId); //20
const int nonleafheader = 3 * sizeof(int) + sizeof(PageId); //16
const int leafEntrySize = sizeof(int) + sizeof(RecordId); //12
const int nonleafEntrySize = sizeof(PageId) + sizeof(int); //8
const int MaxNonLeafEntries = (int)(floor((PageFile::PAGE_SIZE - nonleafheader - sizeof(PageId)) / nonleafEntrySize)); //125
const int MaxLeafEntries = (int)(floor((PageFile::PAGE_SIZE - headerSize) / leafEntrySize)); //83
...
class BTLeafNode {
  public:
    ...
    */
     ...
     * @return 0 if successful. Return an error code if there is an error.
     */
    RC initializeLeaf(int key, const RecordId& rid);

    RC addObject(const void* object, int pos);

    void display(PageId pid);

    //RC readBuffer_to_Object();
...
class BTNonLeafNode {
  public:
    ...
    */
    ...
    RC setTreeHeight(int new_height);

    PageId getRootPid();

    RC setRootPid(PageId pid);

    int getTotalKeys();

    RC setTotalKeys(int total_keys);
